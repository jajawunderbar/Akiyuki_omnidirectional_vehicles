...
def execute_command_1(cycle):
    ...
    motor_b.run_angle(900, -1150, then=Stop.HOLD, wait=True)
    wait(700)
...
while True:
    ...
    if command is not None:
		...
		# way to start point for hub 2:
        if last_cycle == None:    
            wait(4000)
            motor_a.run_angle(900, 820, then=Stop.HOLD, wait=True)
